...
export default function WaterNumber({value}:{value:number|undefined}) {
  ...
  const parts=Array.from(label).map((char,index)=>{
    ...
    cursor+=width+5;
    return glyph?<svg key={index} x={x} y="0" width={width} height="330" viewBox={`${glyph[0]} 178 ${width} 330`} overflow="hidden"><image href="/water-digits.webp" width="2172" height="724"/></svg>:<circle key={index} cx={x+24} cy="293" r="17" fill="#a3eee5" stroke="#edffff" strokeWidth="4"/>;
  });

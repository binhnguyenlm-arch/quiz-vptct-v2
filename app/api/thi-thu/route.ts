...
export async function GET(){
 ...
 try{const data=top?await rpc({op:'board',board:boardFor(top.id)}):{rows:[]};return NextResponse.json({ready:true,packages,max,...data});}catch{return NextResponse.json({ready:false,packages,max,rows:[],message:'Chưa kết nối được bảng kết quả. Vui lòng thử lại sau.'},{status:503});}
}
export async function POST(req:NextRequest){
 ...
 if(body.op==='start'){
  const pkg=availablePackages(bank.questions.length).find(p=>p.id===body.packageId);const name=normalizeName(typeof body.name==='string'?body.name:'');const error=nameError(name);

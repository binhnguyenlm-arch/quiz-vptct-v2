...
export async function GET(req:NextRequest){
 try{if(req.nextUrl.searchParams.get('view')==='round-completed'){const id=req.nextUrl.searchParams.get('id')||'';if(!/^[0-9a-f-]{36}$/i.test(id))return NextResponse.json({error:'Đợt không hợp lệ.'},{status:400});const rounds=await table('rounds',`id=eq.${id}&deleted_at=is.null&state=in.(open,closed)&select=id`);if(!rounds.length)return NextResponse.json({error:'Đợt không còn được công bố.'},{status:404});const records=await table('completed',`round_id=eq.${id}&select=person_id,name_snapshot,completed_at&order=completed_at.asc,person_id.asc`);const completed=(await completionGroups(records)).map(c=>({name_snapshot:c.name_snapshot,completed_at:c.completed_at,audience:c.audience}));return NextResponse.json({completed},{headers:{'Cache-Control':'no-store'}});}if(req.nextUrl.searchParams.get('view')==='rounds'){const rounds=await table('rounds','deleted_at=is.null&state=in.(open,closed)&select=id,title,year,state&order=created_at.desc,id.desc');const ids=rounds.map(r=>r.id);const [,records]=await Promise.all([roundTotals(rounds),ids.length?table('completed',`round_id=in.(${ids.join(',')})&select=round_id,person_id`):Promise.resolve([])]);const completed=await completionGroups(records);for(const r of rounds){r.party_completed=completed.filter(c=>c.round_id===r.id&&c.audience==='party').length;r.public_completed=completed.filter(c=>c.round_id===r.id&&c.audience==='public').length;}return NextResponse.json({rounds},{headers:{'Cache-Control':'no-store'}});}if(req.nextUrl.searchParams.get('view')==='accounts'){const accounts=await table('people','active=eq.true&deleted_at=is.null&select=id,name,username,role&order=name.asc,username.asc');return NextResponse.json({accounts},{headers:{'Cache-Control':'no-store'}});}const me=await identity(req);if(req.nextUrl.searchParams.get('view')==='login')return NextResponse.json({me,rounds:[]},{headers:{'Cache-Control':'no-store'}});const directory=await sb('/rest/v1/rpc/learning_directory','POST',{});if(!me){const rounds=await table('rounds','deleted_at=is.null&state=in.(open,closed)&select=id,title,year,state,audience&order=year.desc,created_at.desc');const ids=rounds.map(r=>r.id);const completed=ids.length?await table('completed',`round_id=in.(${ids.join(',')})&select=round_id,person_id,name_snapshot,completed_at&order=completed_at.asc`):[];await roundTotals(rounds);return NextResponse.json({me:null,rounds,completed:await completionGroups(completed),directory},{headers:{'Cache-Control':'no-store'}});}

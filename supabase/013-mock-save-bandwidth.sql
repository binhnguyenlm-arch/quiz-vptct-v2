-- Run after 011-ranking-avatars.sql. No data deleted.
begin;
...
begin
 ...
 -- Active saves only acknowledge persistence. Start/resume and final results keep their full payload.
 if p->>'op'='save' then
  return jsonb_build_object('state','active','id',a.id,'revision',a.revision,'serverNow',now_at);
 end if;

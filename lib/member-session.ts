...
export async function memberSession(req:NextRequest):Promise<{name:string}|null>{
 ...
 const user=await get('/auth/v1/user',token);const people=await get(`/rest/v1/learning_people?id=eq.${encodeURIComponent(user.id)}&active=eq.true&select=name`,key);if(!people[0])throw Error('SESSION');return {name:people[0].name};
}
